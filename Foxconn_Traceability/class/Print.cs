...
namespace Foxconn_Traceability
{
    class Print
    {
        ...
        public void Imprimir(string serial, string modelo, int quantidade, int totalImpresso, bool etiquetaDupla, bool UsarImpressoraPadrao, string Impressora)
        {
            ...
            if (modelo.Equals("TG1692BR") || modelo.Equals("TG1692A") || modelo.Equals("TG3442"))
            {
                //Impressora ZEBRA
                #region CÓDIGO  ZPL
            ...
            else if (modelo.Equals("TG1692A-SMT") || modelo.Equals("TG3442-SMT"))
            {
                //Impressora ZEBRA
                #region CÓDIGO  ZPL
        ...
        public void Etiqueta_CodeSoft(List<String> Lista_Serial, string modelo, int quantidade, int totalImpresso, bool etiquetaDupla, string Impressora, string snHorizontal, string snVertical, string Wo)
        {
            ...
            if (lista_row_Count > 0)
            {
                ...
                if (etiquetaDupla)
                {
                    ...
                    //
                    if ((modelo.Equals("TG3442-SMT")) || (modelo.Equals("TG1692A-SMT")) || (modelo.Equals("TG1692BR-SMT")) || (modelo.Equals("ROKU-MAC")) || (modelo.Equals("ROKU-PANEL")) || (modelo.Equals("ROKU-SMT")))
                    {
                        for (int row = 0; row < quantidade; row++)//quantidade vezes para imprimir
                        {
                            ...
                            for (int index = 0; index < row_count_total_String; index++)
                            {
                                ...
                                if (index == (row_count_total_String - 1))//encerra leitura da linha da lista
                                {
                                    ...
                                    {
                                        arquivoCodeSoft = "TG3442_SMT_DUPLA.lab";
                                    }
                                    else if (modelo.Equals("TG1692BR-SMT"))
                                    {
                ...
                else
                {
                    ...
                    {
                        arquivoCodeSoft = "TG3442_SMT_SIMPLES.lab";
                    }
                    else if (modelo.Equals("TG1692BR-SMT"))
                    {
                    ...
                    {
                        arquivoCodeSoft = "ROKU_PANEL_SIMPLES.Lab";
                    }
                    else if (modelo.Equals("ROKU-SMT"))
                    {
                    ...
                    try
                    {
                        ...
                        serial = Lista_Serial[0].ToString();//Lista_Serial[index].ToString();
                        //VARIAVEL CODESOFT LABEL 
                        lbl.ActiveDocument.Variables.FormVariables.Item("SERIAL").Value = serial;
                        if (modelo.Equals("ROKU-GIFIT_BOX"))
                        {
                            lbl.ActiveDocument.Variables.FormVariables.Item("LOT").Value = Lista_Serial[1].ToString();
                        }
                        //
                        if (modelo.Equals("TG3442-SMT"))
                        {
                            lbl.ActiveDocument.Variables.FormVariables.Item("WO").Value = Wo;//somente os 4 últimos digitos da wo
                        }
                        ...
                        if ((!string.IsNullOrEmpty(snHorizontal)) && (!string.IsNullOrEmpty(snVertical)))
                        {
                            ...
                            if (modelo.Equals("ROKU-SMT"))
                            {
                                lbl.ActiveDocument.Variables.FormVariables.Item("WO").Value = snVertical;
                            }
                            else if (modelo.Equals("ROKU-MAC"))
                            {
