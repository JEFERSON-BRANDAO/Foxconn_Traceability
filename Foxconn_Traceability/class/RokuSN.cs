    ...
    class RokuSN
    {
        public string Mac(string sn)
        {
            ...
            try
            {
                try
                {
                    ...
                    string sql = string.Format("SELECT MAC FROM sfcconfig.mac_address WHERE MAC ='{0}'", sn);
                    //                           
        ...
        public string Modelo(string sn)
        {
            ...
            try
            {
                try
                {
                    ...
                    //                                    WHERE DATA1= 'ROKU_SN' AND DATA5 LIKE '" + sn.Remove(1, sn.Length - 1) + "%'";//Extrai somente 1ro caractere do serial
                    //                            WHERE DATA1= 'ARRIS_SN' AND DATA5 LIKE '" + sn.Remove(1, sn.Length - 1) + "%'";//Extrai somente 1ro caractere do serial
                    string sql = string.Format(@"SELECT DATA2 AS MODELO FROM R_AP_TEMP
                                    WHERE DATA1= 'ROKU_SN' AND DATA5 LIKE '{0}%'", sn.Remove(1, sn.Length - 1));//Extrai somente 1ro caractere do serial
                    Objconn.SetarSQL(sql);
                    Objconn.Executar();
        ...
        public string generate_Roku_SN(int seq, string modelo, string valorInicial)
        {
            ...
            else if (modelo.Equals("ROKU-SMT"))//ROKU-BLANK
            {
                ...
                string SSSS = convertToAlphaSequencia(s1, s2, s3, s4);//Sequential numbers assigned by factory
                //SSSS = SSSS.Equals("0000") ? "0001" : SSSS;
        ...
        public String pref_Roku_SN(string modelo)
        {
            ...
            if (modelo.Equals("ROKU-SMT"))//ROKU-BLANK
            {
                ...
                */
                ...
                // int num_week = cal.GetWeekOfYear(date1, dfi.CalendarWeekRule, DayOfWeek.Monday);//número da semana do ano

                DateTime date1 = new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day);
                CultureInfo ciCurr = CultureInfo.CurrentCulture;
                int num_week = ciCurr.Calendar.GetWeekOfYear(date1, CalendarWeekRule.FirstFourDayWeek, DayOfWeek.Monday);

                string year = DateTime.Now.Year.ToString("X").Remove(0, 2);//hexadecimal 2 últimos digitos
                int week_year = (num_week + int.Parse(year)) + ((int.Parse(DateTime.Now.Year.ToString().Remove(0, 2))) + 103);//add ((int.Parse(DateTime.Now.Year.ToString().Remove(0, 2))) + 102) para valor do prefixo da label continuar da sequencia de onde parou no proximo ano               //add 41 devido existir registros de etiquetas com valor 24 a mais
        ...
        public String convertToAlphaSequencia(int valor1, int valor2, int valor3, int valor4)
        {
            ...
            if (atualizado == "C2")
            {
                if ((c2 == "A") || (c2 == "0"))
                {
                    ...
                    else
                    {
                        ...
                        c1 = caracteresValidos[valor1 + 1].ToString();//NOVA SEQUÊNCIA
                    }
                }
                else
                {
            ...
            novaSequencia = novaSequencia == "ZZZZ" ? "0001" : novaSequencia;
            ...
            try
            {
                                ...
                                for (int index = 0; index < qty; index++)
                                {
                                    ...
                                    if (!string.IsNullOrEmpty(serial))
                                    {
                                        ...
                                        try
                                        {
                                            ...
                                            string sql = string.Format("UPDATE sfcconfig.mac_address SET USED = 1, LAST_UPDATE = sysdate  WHERE MAC='{0}'", serial);
                                            Objconn.SetarSQL(sql);
                                            Objconn.Executar();
                                        }
                                        finally
                ...
                else if (modelo.Equals("ROKU-PANEL"))
                {
                    try
                    {
                        ...
                        string sql = string.Format(@"SELECT TRIM(DATA5) AS SERIAL FROM R_AP_TEMP
                                   WHERE DATA1='ROKU_SN' AND DATA2='{0}'", modelo);
                        Objconn.SetarSQL(sql);
                        Objconn.Executar();
                    }
                    finally
                    ...
                    try
                    {
                        ...
                        string sql = string.Format(@"SELECT TRIM(MAX(SYSSERIALNO)) AS SERIAL FROM MFWORKSTATUS
                                   WHERE SYSSERIALNO LIKE '{0}%'", pref_Roku_SN(modelo));
                        Objconn.SetarSQL(sql);
                        Objconn.Executar();
                    }
                    finally
                    ...
                    if (list.Count > 0)
                    {
                        ...
                        if (temp == 1)
                        {
                            ...
                            sql = string.Format("UPDATE  R_AP_TEMP SET DATA5 = '{0}', WORK_TIME = SYSDATE WHERE DATA2='{1}'", serial, modelo);
                        }
                        else
                        {
                            ...
                            sql = string.Format(@"INSERT INTO  R_AP_TEMP (DATA1, DATA2, DATA3, DATA4, DATA5, DATA6, DATA7, WORK_TIME)" +
                          "VALUES ('ROKU_SN', '{0}', 1, 'SERIAL', '{1}', 'S/N', 0, SYSDATE)", modelo, serial);
                        }
                        //
                        try
                        {
                ...
                else if (modelo.Equals("ROKU-SMT"))//("ROKU-BLANK"))
                {
                    try
                    {
                        ...
                        //                        string sql = @"SELECT TRIM(DATA5) AS SERIAL FROM R_AP_TEMP
                        //                                   WHERE DATA1='ROKU_SN' AND DATA2='" + modelo + "'";
                        string sql = string.Format("SELECT TRIM(DATA5) AS SERIAL FROM R_AP_TEMP WHERE DATA1='ROKU_SN' AND DATA2='{0}'", modelo);
                        Objconn.SetarSQL(sql);
                        Objconn.Executar();
                    }
                    finally
        ...
        public String get_lastSN(string modelo)
        {
            ...
            try
            {
                try
                {
                    ...
                    //                            WHERE DATA1= 'ROKU_SN' AND DATA2 ='" + modelo + "'";
                    string sql = string.Format("SELECT DATA5 AS SERIAL FROM R_AP_TEMP  WHERE DATA1= 'ROKU_SN' AND DATA2 ='{0}'", modelo);
                    Objconn.SetarSQL(sql);
                    Objconn.Executar();
        ...
        public String get_Wo(string wo)
        {
            ...
            try
            {
                try
                {
                    ...
                    //                                    "AND WORKORDERNO='" + wo + "')";
                    string sql = string.Format(@"SELECT COUNT(*) QTD FROM MFWORKORDERSPACE 
                                    WHERE WORKORDERNO IN (SELECT WORKORDERNO FROM MFWORKORDER WHERE SKUNO='RU9026000643' AND RELEASED=1 AND JOBSTARTED=1 AND CLOSED=0
                                    AND FINISHEDQTY <= (SELECT COUNT(*)QTD FROM MFWORKORDERSPACE WHERE WORKORDERNO='{0}') " +
                                   "AND WORKORDERNO='{1}')", wo, wo);
                    Objconn.SetarSQL(sql);
                    Objconn.Executar();
        ...
        public bool Wo_Valida(string wo)
        {
            ...
            try
            {
                try
                {
                    ...
                    string sql = string.Format(@"SELECT WORKORDERNO FROM MFWORKORDER 
                                  WHERE SKUNO='RU2326000327' 
                                  AND RELEASED=1 AND JOBSTARTED=1 AND CLOSED=0 AND WORKORDERNO='{0}'", wo);
                    Objconn.SetarSQL(sql);
                    Objconn.Executar();
        ...
        public List<Gifit> Gifit_Box(string Mac)
        {
            OleDbConnect Objconn = new OleDbConnect();
            List<Gifit> lista = new List<Gifit>();
            Gifit item = new Gifit();
            string mac = string.Empty;
            ...
            try
            {
                ...
                Objconn.Parametros.Clear();
                //
                //                string sql = @"SELECT SYSSERIALNO MAC,      
                //                               CASE (select LENGTH(WORKORDERNO) TAMANHO from mfworkstatus WHERE SYSSERIALNO='" + Mac + @"')
                //                                 WHEN 12 THEN  SUBSTR(WORKORDERNO, 3, LENGTH(WORKORDERNO) - 1)
                //                               ELSE WORKORDERNO   
                //                               END LOT
                //                               FROM mfworkstatus where SYSSERIALNO='" + Mac + "'";
                //
                //                string sql = @"SELECT SYSSERIALNO MAC,     
                //                                   CASE (select SUBSTR(WORKORDERNO,0,4) PREFIXO from mfworkstatus WHERE SYSSERIALNO='" + Mac + @"')                        
                //                                      WHEN '0000' THEN  SUBSTR(WORKORDERNO, 5, LENGTH(WORKORDERNO) - 1)
                //                                   ELSE SUBSTR(WORKORDERNO, 3, LENGTH(WORKORDERNO) - 1) 
                //                                   END LOT
                //                                   FROM mfworkstatus where SYSSERIALNO='" + Mac + "'";
                string sql = string.Format(@"SELECT SYSSERIALNO MAC,     
                                   CASE (select SUBSTR(WORKORDERNO,0,4) PREFIXO from mfworkstatus WHERE SYSSERIALNO='{0}')                        
                                      WHEN '0000' THEN  SUBSTR(WORKORDERNO, 5, LENGTH(WORKORDERNO) - 1)
                                   ELSE SUBSTR(WORKORDERNO, 3, LENGTH(WORKORDERNO) - 1) 
                                   END LOT
                                   FROM mfworkstatus where SYSSERIALNO='{1}'", Mac, Mac);
                //
                Objconn.SetarSQL(sql);
                Objconn.Executar();
                ...
                if (Objconn.Tabela.Rows.Count > 0)
                {
                    mac = Objconn.Tabela.Rows[0]["MAC"].ToString();
                    wo = Objconn.Tabela.Rows[0]["LOT"].ToString();
                    //
                    item.LOT = wo;
                    item.MAC = mac;
                    //
        ...
        public List<EtiquetaMAC> Get_MAC_WO(string Mac)
        {
            OleDbConnect Objconn = new OleDbConnect();
            List<EtiquetaMAC> lista = new List<EtiquetaMAC>();
            EtiquetaMAC item = new EtiquetaMAC();
            //
            string mac = string.Empty;
            string wo = string.Empty;
            //
            try
            {
                Objconn.Conectar();
                Objconn.Parametros.Clear();
                //string sql = @"SELECT SYSSERIALNO AS MAC, WORKORDERNO AS WO FROM MFWORKSTATUS WHERE SYSSERIALNO='" + Mac + "'";
                string sql = string.Format("SELECT SYSSERIALNO AS MAC, WORKORDERNO AS WO FROM MFWORKSTATUS WHERE SYSSERIALNO='{0}'", Mac);
                //
                Objconn.SetarSQL(sql);
                Objconn.Executar();
                //
                ...
                {
                    mac = Objconn.Tabela.Rows[0]["MAC"].ToString();
                    wo = Objconn.Tabela.Rows[0]["WO"].ToString();
                    //
                    item.MAC = mac;
                    item.WO = wo;
                    //
